    ...
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            ...
            @if(auth()->user()->role && auth()->user()->role->name === 'super_admin')
                <a href="{{ route('hospitals.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">
                    Add Hospital
                </a>
            @endif
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-sm text-gray-500">{{ $hospital->type }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $hospital->city }}, {{ $hospital->state }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('hospitals.show', $hospital) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                            @if(auth()->user()->role && auth()->user()->role->name === 'super_admin')
                                <a href="{{ route('hospitals.edit', $hospital) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                <form action="{{ route('hospitals.destroy', $hospital) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this hospital?')">Delete</button>
                                </form>
                            @endif
                        </td>
                    </tr>
